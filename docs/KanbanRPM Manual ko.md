...
`Timeline` view는 Laminar-style layout입니다. 왼쪽 `Perpetual` sidebar, 상단 range/search/scope controls, horizontal date columns, `Memo` row를 사용합니다. `Due date`, `Next review`, 날짜가 있는 unchecked small action, `Perpetual` routine marker를 표시합니다. marker를 누르면 해당 living document를 엽니다. `Memo` 또는 day cell `+`를 누르면 `KanbanRPM Workspace/timeline/YYYY-MM-DD.md`를 생성하거나 엽니다.
